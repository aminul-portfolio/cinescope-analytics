...
- Multi-field ranking for top-rated content: ordered by average rating, review count, and views
...
|---|---|
| ETL pipeline logic | `movie/management/commands/build_daily_metrics.py` |
| Gold table + run log models | `movie/models.py` → `DailyMetric`, `ETLRunLog` |
| Dashboard view and KPI queries | `movie/views.py` → `analytics_dashboard` |
| Raw event capture | `movie/models.py` → `Event` |
| Top-rated ranking query | `movie/views.py` → `top_movies_rated` |

Run `python manage.py build_daily_metrics` to execute the ETL pipeline and write to both `DailyMetric` and `ETLRunLog`.
...
- Top tables: movies by watches, movies by favourites, top-rated using multi-field ranking
...
```bash
git clone https://github.com/aminul-portfolio/cinescope-analytics
...
|---|---|
| Data Analyst | KPI dashboard, funnel analysis, CSV exports, multi-field ranking |
